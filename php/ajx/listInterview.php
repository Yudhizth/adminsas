...
if(@$_GET['type'] == 'intreview'){
    $filter = $_GET['filter'];
    $type = $_GET['tipe'];

    $records_per_page = 20;
    $query = "SELECT tb_karyawan.no_ktp, tb_karyawan.no_NIK, tb_karyawan.nama_depan, tb_karyawan.nama_belakang, tb_info_interview.kd_interview, tb_info_interview.date_interview, tb_info_interview.detail, tb_info_interview.kd_admin, tb_info_interview.status FROM tb_karyawan
            LEFT OUTER JOIN tb_info_interview ON tb_info_interview.no_ktp=tb_karyawan.no_ktp
